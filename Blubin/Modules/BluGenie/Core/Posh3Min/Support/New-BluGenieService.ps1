#region New-BluGenieService (Function)
Function New-BluGenieService
{
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        Command: <Command Goes Here> -OutUnEscapedJSON
    ...
    .EXAMPLE
        Command: <Command Goes Here> -OutYaml
        Description: Return a detailed function report in YAML format
        Notes:  The OutUnEscapedJSON is used to Beautify the JSON return and not Escape any Characters.  Normal return data is a Hash Table.

    .EXAMPLE
        Command: <Command Goes Here> -ReturnObject
...
#>
    ...
    #region Create Return hash
        $HashReturn = @{}
        $HashReturn['NewBGService'] = @{}
        $StartTime = $(Get-Date -ErrorAction SilentlyContinue)
        $HashReturn['NewBGService'].StartTime = $($StartTime).DateTime
        $HashReturn['NewBGService'].ParameterSetResults = @()
        $HashReturn['NewBGService']['Service'] = @()
    #endregion Create Return hash

    #region Parameter Set Results
        $HashReturn['NewBGService'].ParameterSetResults += $PSBoundParameters
    ...
    #region Main
        ...
        )
        {
            ...
            If
            (
                Test-Path -Path $('{0}\BGService.exe' -f $BGServicePath)
            )
            {
                $CurServiceObj.BinaryFound = 'True'
                ...
                )
                {
                    ...
                    )
                    {
                        ...
                        Try
                        {
                            $null = Invoke-Expression -Command "sc.exe delete $($CurServiceObj.Service)"
                        }
                        Catch
                        {
                            #Emply Catch Statement
                        }
                    }

                    Try
                    {
                        $Null = New-Service -Name $($CurServiceObj.Service) -BinaryPathName $('{0}\BGService.exe' -f $BGServicePath) `
                            -StartupType Automatic -ErrorAction Stop

                        $Null = Start-Service -Name $($CurServiceObj.Service) -Erroraction Stop
                ...
                Else
                {
                    ...
                    )
                    {
                        If
                        (
                            $(Get-Service -Name $($CurServiceObj.Service) | Select-Object -ExpandProperty 'Status') -ne 'Running'
                        )
                        {
                            $Null = Start-Service -Name $($CurServiceObj.Service) -Erroraction Stop
                        }
    ...
    #region Output
        ...
        $HashReturn['NewBGService'].EndTime = $($EndTime).DateTime
        $HashReturn['NewBGService'].ElapsedTime = $(New-TimeSpan -Start $StartTime -End $EndTime -ErrorAction SilentlyContinue)`
        ...
        )
        {
            ...
            $null = $HashReturn['NewBGService'].Remove('StartTime')
            $null = $HashReturn['NewBGService'].Remove('ParameterSetResults')
            $null = $HashReturn['NewBGService'].Remove('EndTime')
            $null = $HashReturn['NewBGService'].Remove('ElapsedTime')
        }

        #region Output Type
            $ResultSet = $CurServiceObj
            $HashReturn['NewBGService']['Service'] += $CurServiceObj
...
#endregion New-BluGenieService (Function)