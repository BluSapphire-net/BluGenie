#Requires -Version 3
#region Stop-BluGenieService (Function)
Function Stop-BluGenieService
{
<#
    .SYNOPSIS
...
#>
    ...
    #region Create Return hash
        $HashReturn = @{}
        $HashReturn['StopBGService'] = @{}
        $StartTime = $(Get-Date -ErrorAction SilentlyContinue)
        $HashReturn['StopBGService'].StartTime = $($StartTime).DateTime
        $HashReturn['StopBGService'].ParameterSetResults = @()
        $HashReturn['StopBGService']['Service'] = @()
    #endregion Create Return hash

    #region Parameter Set Results
        $HashReturn['StopBGService'].ParameterSetResults += $PSBoundParameters
    ...
    #region Output
        ...
        $HashReturn['StopBGService'].EndTime = $($EndTime).DateTime
        $HashReturn['StopBGService'].ElapsedTime = $(New-TimeSpan -Start $StartTime -End $EndTime -ErrorAction SilentlyContinue)`
        ...
        )
        {
            ...
            $null = $HashReturn['StopBGService'].Remove('StartTime')
            $null = $HashReturn['StopBGService'].Remove('ParameterSetResults')
            $null = $HashReturn['StopBGService'].Remove('EndTime')
            $null = $HashReturn['StopBGService'].Remove('ElapsedTime')
        }

        #region Output Type
            $ResultSet = $CurServiceObj
            $HashReturn['StopBGService']['Service'] += $CurServiceObj
...
#endregion Stop-BluGenieService (Function)