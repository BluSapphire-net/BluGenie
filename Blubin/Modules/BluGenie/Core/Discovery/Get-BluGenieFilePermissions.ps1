#region Get-BluGenieFilePermissions (Function)
Function Get-BluGenieFilePermissions
{
<#
    .SYNOPSIS
    ...
    #region begin
        begin
        {
            ...
            #region Create Return hash
                $HashReturn = @{}
                $HashReturn['GetFilePermissions'] = @{}
                $StartTime = $(Get-Date -ErrorAction SilentlyContinue)
                $HashReturn['GetFilePermissions'].StartTime = $($StartTime).DateTime
                $HashReturn['GetFilePermissions'].ParameterSetResults = @()
                $HashReturn['GetFilePermissions']['Items'] = @()
                $HashReturn['GetFilePermissions']['CachePath'] = $CachePath
            #endregion Create Return hash

            #region Parameter Set Results
                $HashReturn['GetFilePermissions'].ParameterSetResults += $PSBoundParameters
                ...
                #region Process Each Instance in Path
                    $_ | ForEach-Object `
                    {
                        ...
                        #region Query Permissions
                            ...
                            $CurPermObj = New-Object -TypeName PSObject -Property $NewObjProps
                            $CurPermObj.Path = $($Path)
                            $CurPermObj.LiteralPath = $LPath

                            $Error.Clear()

                            Try
                            {
                                $CurChildQueryACL = $(Get-Acl -Path $($LPath) -ErrorAction SilentlyContinue | `
				                                Select-Object -Property Owner,Access)

                                $CurPermObj.OnDisk = 'TRUE'
                                $CurPermObj.Owner = $CurChildQueryACL.Owner
                                
                                
                                $CurChildQueryACL.Access | ForEach-Object `
                                -Process `
                                {
                                    $CurChildAccess = $_
                                    Try
                                    {
                                        $ArrCurPermsObj += New-Object -TypeName PSObject -Property @{
                                            FileSystemRights = ''
                                            AccessControlType = ''
                                            IdentityReference = ''
                                            IsInherited = ''
                                        ...
                                        $ArrCurPermsObj.FileSystemRights = $($CurChildAccess.FileSystemRights | Out-String -ErrorAction SilentlyContinue).trim()
                                        $ArrCurPermsObj.AccessControlType = $($CurChildAccess.AccessControlType | Out-String -ErrorAction SilentlyContinue).trim()
                                        $ArrCurPermsObj.IdentityReference = $($CurChildAccess.IdentityReference.Value | Out-String -ErrorAction SilentlyContinue).trim()
                                        $ArrCurPermsObj.IsInherited = $($CurChildAccess.IsInherited | Out-String -ErrorAction SilentlyContinue).trim()
                                        $ArrCurPermsObj.InheritanceFlags = $($CurChildAccess.InheritanceFlags | Out-String -ErrorAction SilentlyContinue).trim()
                                        $ArrCurPermsObj.PropagationFlags = $($CurChildAccess.PropagationFlags | Out-String -ErrorAction SilentlyContinue).trim()

                                        $CurArrPerms += $ArrCurPermsObj
                                    }
                                    Catch
                                    {
                                        #Nothing
                            ...
                            Catch
                            {
                                $CurPermObj.Comment = $error[0] | Out-String
                                $Error.Clear()
                            }

                            $CurPermObj.Permissions += $CurArrPerms
                            ...
                            )
                            {
                                ...
                                $CurPermObj | ConvertTo-Yaml | Out-File $CachePath -Append -Force
                            }
                            Else
                            {
                                $ArrPermsData += $CurPermObj
                            }
    ...
    #region end
        end
        {
            ...
            #region Build Return Array
                ...
                {
                    $HashReturn['GetFilePermissions']['Items'] += Get-Content -Path $CachePath | ConvertFrom-Yaml -AllDocuments
                    If
                    (
                        $RemoveCache
                    )
                ...
                Else
                {
                    $HashReturn['GetFilePermissions']['Items'] += $ArrPermsData
                }
            #endregion Build Return Array
        
            #region Output
                $EndTime = $(Get-Date -ErrorAction SilentlyContinue)
                $HashReturn['GetFilePermissions'].EndTime = $($EndTime).DateTime
                $HashReturn['GetFilePermissions'].ElapsedTime = $(New-TimeSpan -Start $StartTime -End $EndTime -ErrorAction SilentlyContinue)`
                ...
                #region Output Type
                    $ResultSet = $($HashReturn['GetFilePermissions']['Items'])
...
#endregion Get-BluGenieFilePermissions (Function)