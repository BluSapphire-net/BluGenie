#region Get-BluGenieLoadedRegHives (Function)
    function Get-BluGenieLoadedRegHives
    {
    <#
        .SYNOPSIS
        ...
        .NOTES
            ...
            * Latest Build Version      : 1903.1401
            * Comments                  :
            * Dependencies              :
                                            ~
            * Build Version Details     :
                                            ~ 1903.0601: * [Michael Arroyo] Posted
                                            ~ 1903.1401: * [Michael Arroyo] Added PSPath as a new Object entry
                                                            * [Michael Arroyo] Added RegPrefix as a new Object entry
    #>
        [Alias('Get-LoadedRegHives')]
        Param
        (
        ...
        #region Query the HKEY_USERS Key
            $EnumHKU = Get-ChildItem -Path HKU:\ -ErrorAction SilentlyContinue | Split-Path -Leaf -ErrorAction SilentlyContinue
        #endregion

        #region Build out object
            $ArrEnumHKU = @()

            $EnumHKU | ForEach-Object `
            -Process `
            {
                $ObjEnumHKU = New-Object -TypeName psobject
                $CurEnumHKU = $_

                $ObjEnumHKU | Add-Member -MemberType NoteProperty -Name 'UserName' -Value $CurEnumHKU
                $ObjEnumHKU | Add-Member -MemberType NoteProperty -Name 'ProfilePath' -Value $(Get-ItemProperty -Path $('HKU:\{0}\Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders' -f $CurEnumHKU) -Name 'Local AppData' -ErrorAction SilentlyContinue | Select-Object -ExpandProperty 'Local AppData' | Split-Path -Parent | Split-Path -Parent)
                $ObjEnumHKU | Add-Member -MemberType NoteProperty -Name 'UserHive' -Value $('HKEY_USERS\{0}' -f $CurEnumHKU)
                $ObjEnumHKU | Add-Member -MemberType NoteProperty -Name 'PSPath' -Value $('HKU:\{0}' -f $CurEnumHKU)
                $ObjEnumHKU | Add-Member -MemberType NoteProperty -Name 'RegPrefix' -Value 'HKEY_USERS'

                If
                (
                    $CurEnumHKU -match '^S-1'
                )
                {
                    $UserNameFromSID = Convert-SID2UserName -SID $CurEnumHKU -ErrorAction SilentlyContinue

                    If
                    (
                        -Not $($UserNameFromSID -match '^Oops')
                    )
                    {
                        $ObjEnumHKU | Add-Member -MemberType NoteProperty -Name 'UserFromSID' -Value $UserNameFromSID
                    }
                    Else
                    {
                        $ObjEnumHKU | Add-Member -MemberType NoteProperty -Name 'UserFromSID' -Value ''
                    }
                }
                Else
                {
                    $ObjEnumHKU | Add-Member -MemberType NoteProperty -Name 'UserFromSID' -Value $CurEnumHKU
                }

                If
                (
                    $ObjEnumHKU.ProfilePath
                )
                {
                    $ObjEnumHKU | Add-Member -MemberType NoteProperty -Name 'LoadedShell' -Value $true
                }
                Else
                {
                    $ObjEnumHKU | Add-Member -MemberType NoteProperty -Name 'LoadedShell' -Value $false
                }

                $ArrEnumHKU += $ObjEnumHKU
            }
        #endregion
        ...
        #region Output
            ...
            Else
            {
                $HashReturn.LoadedRegHives.Hives = $ArrEnumHKU
                $HashReturn.LoadedRegHives.Total = $($ArrEnumHKU.Count)

                Return $HashReturn
            }
        #endregion
    }
#endregion Get-BluGenieLoadedRegHives (Function)