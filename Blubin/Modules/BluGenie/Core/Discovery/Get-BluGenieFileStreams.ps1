#region Get-BluGenieFileStreams (Function)
Function Get-BluGenieFileStreams
{
<#
    .SYNOPSIS
    ...
    #region begin
        begin
        {
            ...
            #region Create Return hash
                $HashReturn = @{}
                $HashReturn['GetFileStreams'] = @{}
                $StartTime = $(Get-Date -ErrorAction SilentlyContinue)
                $HashReturn['GetFileStreams'].StartTime = $($StartTime).DateTime
                $HashReturn['GetFileStreams'].ParameterSetResults = @()
                $HashReturn['GetFileStreams']['Items'] = @()
                $HashReturn['GetFileStreams']['CachePath'] = $CachePath
            #endregion Create Return hash

            #region Parameter Set Results
                $HashReturn['GetFileStreams'].ParameterSetResults += $PSBoundParameters
                ...
                #region Process Each Instance in Path
                    $_ | ForEach-Object `
                    {
                        ...
                        #region Query Streams
                            ...
                            $CurStreamObj = New-Object -TypeName PSObject -Property $NewObjProps
                            $CurStreamObj.Path = $($Path)
                            $CurStreamObj.LiteralPath = $LPath

                            $Error.Clear()

                            Try
                            {
                                $CurQueryStreams = Get-BluGenieFileADS -Path $($LPath) -ReturnObject -ErrorAction SilentlyContinue
                                $CurStreamObj.OnDisk = 'TRUE'
                                
                                If
                                (
                                    $CurQueryStreams.HasADS
                                )
                                {
                                    $CurQueryStreams.ADSName | ForEach-Object `
                                    -Process `
                                    {
                                        $CurStreamContent = New-Object -TypeName PSObject -Property @{
                                            Name = ''
                                            Content = ''
                                        }

                                        $CurStreamContent.Name = $_
                                        $CurStreamContent.Content = $(Get-Content -Path $LPath -Stream $_ | Out-String).trim()

                                        $CurArrStreams += $CurStreamContent

                                        $CurStreamContent = $null
                                    }
                                }
                            }
                            Catch
                            {
                                $CurStreamObj.Comment = $error[0] | Out-String
                                $Error.Clear()
                            }

                            $CurStreamObj.Streams += $CurArrStreams
                            ...
                            )
                            {
                                ...
                                $CurStreamObj | ConvertTo-Yaml | Out-File $CachePath -Append -Force
                            }
                            Else
                            {
                                $ArrStreamData += $CurStreamObj
                            }
    ...
    #region end
        end
        {
            ...
            #region Build Return Array
                ...
                {
                    $HashReturn['GetFileStreams']['Items'] += Get-Content -Path $CachePath | ConvertFrom-Yaml -AllDocuments
                    If
                    (
                        $RemoveCache
                    )
                ...
                Else
                {
                    $HashReturn['GetFileStreams']['Items'] += $ArrStreamData
                }
            #endregion Build Return Array
        
            #region Output
                $EndTime = $(Get-Date -ErrorAction SilentlyContinue)
                $HashReturn['GetFileStreams'].EndTime = $($EndTime).DateTime
                $HashReturn['GetFileStreams'].ElapsedTime = $(New-TimeSpan -Start $StartTime -End $EndTime -ErrorAction SilentlyContinue)`
                ...
                #region Output Type
                    $ResultSet = $($HashReturn['GetFileStreams']['Items'])
...
#endregion Get-BluGenieFileStreams (Function)