#region Remove-BluGenieFile (Function)
Function Remove-BluGenieFile
{
<#
    .SYNOPSIS
        Remove file
    ...
    .EXAMPLE
	    Command: Remove-BluGenieFile -path "C:\Temp\File1.txt","%SystemDrive%\Pagefile.sys"
        Description: 
        Notes: 

    .EXAMPLE
	    Command: Remove-BluGenieFile -path "C:\Temp\File1.txt,"%SystemDrive%\Pagefile.sys"
        Description: Remove file(s) using a Single String Array with a comma separator
        Notes: 

    .EXAMPLE
	    Command: Remove-BluGenieFile -path "C:\Temp\File1.txt;"%SystemDrive%\Pagefile.sys"
        Description: Remove file(s) using a Single String Array with a semicolon separator
        Notes: 

    .EXAMPLE
	    Command: Get-ChildItem -path $env:temp -File | Remove-BluGenieFile
        Description: Remove file(s) using Named value from Pipeline
        Notes: 

    .EXAMPLE
	    Command: Get-ChildItem -path $env:temp -File | Select-Object -ExpandProperty Fullname | Remove-BluGenieFile
        Description: Remove file(s) using value from Pipeline
        Notes: 

    .EXAMPLE
	    Command: Get-FileRemove -path "C:\Temp\File1.txt"
        Description: Remove file(s) using the Function Alias
        Notes: 

    .EXAMPLE
	    Command: Remove-BluGenieFile -path "C:\Temp\File1.txt" -UseCache
        Description: Cache found objects to disk to not over tax Memory resources
        Notes: By default the Cache location is %SystemDrive%\Windows\Temp

    .EXAMPLE
	    Command: Remove-BluGenieFile -path "C:\Temp\File1.txt" -UseCache -RemoveCache
        Description: Remove Cache data
        Notes: 

    .EXAMPLE
	    Command: Remove-BluGenieFile -path "C:\Temp\File1.txt" -UseCache -CachePath $Env:Temp
        Description: Change the Cache path to the current users Temp directory
        Notes: By default the Cache location is %SystemDrive%\Windows\Temp

    .EXAMPLE
	    Command: Get-ChildItem -path $env:temp -File | Remove-BluGenieFile -UseCache -ClearGarbageCollecting
        Description: Scan large directories and limit the memory used to track data
        Notes: 

    .EXAMPLE
	    Command: Remove-BluGenieFile -Help
        Description: Call Help Information
        Notes: If Help / WalkThrough is setup as a parameter, this script will be called to setup the Dynamic Help Menu if not the normal 
Get-Help will be called with the -Full parameter

    .EXAMPLE
	    Command: Remove-BluGenieFile -WalkThrough
        Description: Call Help Information [2]
        Notes: If Help / WalkThrough is setup as a parameter, this script will be called to setup the Dynamic Help Menu if not the normal 
Get-Help will be called with the -Full parameter

    .EXAMPLE
	    Command: Remove-BluGenieFile -path "C:\Temp\File1.txt" -OutUnEscapedJSON
    ...
    .EXAMPLE
	    Command: Remove-BluGenieFile -path "C:\Temp\File1.txt" -OutYaml
        Description: Return a detailed function report in YAML format
        Notes:  The OutUnEscapedJSON is used to Beautify the JSON return and not Escape any Characters.  Normal return data is a Hash Table.

    .EXAMPLE
	    Command: Remove-BluGenieFile -path "C:\Temp\File1.txt" -ReturnObject
        Description: Return Output as a Object
        Notes:  The ReturnObject is used to return a PowerShell Object.  Normal return data is a Hash Table.
                This parameter is also used with the ForMat

    .EXAMPLE
	    Command: Remove-BluGenieFile -path "C:\Temp\File1.txt" -ReturnObject -FormatView Yaml
    ...
    .NOTES
        ...
        • [Build Version Details]
            ...
            o 20.12.1201: * [Michael Arroyo] Posted
#>
    [cmdletbinding()]
    [Alias('Remove-File')]
    #region Parameters
    ...
    #region begin
        begin
        {
            ...
            #region Create Return hash
                $HashReturn = @{}
                $HashReturn['RemoveFile'] = @{}
                $StartTime = $(Get-Date -ErrorAction SilentlyContinue)
                $HashReturn['RemoveFile'].StartTime = $($StartTime).DateTime
                $HashReturn['RemoveFile'].ParameterSetResults = @()
                $HashReturn['RemoveFile']['Items'] = @()
                $HashReturn['RemoveFile']['CachePath'] = $CachePath
            #endregion Create Return hash

            #region Parameter Set Results
                $HashReturn['RemoveFile'].ParameterSetResults += $PSBoundParameters
                ...
                #region Process Each Instance in Path
                    $_ | ForEach-Object `
                    {
                        ...
                        #region Query ADS Names
                            ...
                            $CurRemoveObj = New-Object -TypeName PSObject -Property $NewObjProps
                            $CurRemoveObj.Path = $($Path)
                            $CurRemoveObj.LiteralPath = $LPath

                            $Error.Clear()

                            $CurRemoveFileCheck = Test-BluGenieIsFileLocked -Path $LPath -ReturnObject -ErrorAction SilentlyContinue

                            If
                            (
                                $CurRemoveFileCheck.IsLocked
                            )
                            {
                                $CurRemoveObj.IsLocked = 'TRUE'
                            }

                            If
                            (
                                $CurRemoveFileCheck.Exists
                            )
                            {
                                $CurRemoveObj.OnDisk = 'TRUE'
                            }
                            ...
                            {
                                $CurRemoveObj.LockedProcess += $(Get-BluGenieLockingProcess -Path $LPath -ReturnObject `
                            ...
                            Else
                            {
                                Try
                                {
                                    $CurRemoveFile = [System.IO.File]::Delete("$LPath")

                                    $CurRemoveObj.Removed = 'TRUE'
                                }
                                Catch
                                {
                                    $Error[0] | Select-Object -ExpandProperty Exception | Out-String
                                    $Error.Clear()
                                }
                            }
                            ...
                            )
                            {
                                ...
                                $CurRemoveObj | ConvertTo-Yaml | Out-File $CachePath -Append -Force
                            }
                            Else
                            {
                                $ArrRemoveFileData += $CurRemoveObj
                            }

                            $CurRemoveObj = $null
                            $CurRemoveFileCheck = $null
    ...
    #region end
        end
        {
            ...
            #region Build Return Array
                ...
                {
                    $HashReturn['RemoveFile']['Items'] += Get-Content -Path $CachePath | ConvertFrom-Yaml -AllDocuments
                    If
                    (
                        $RemoveCache
                    )
                ...
                Else
                {
                    $HashReturn['RemoveFile']['Items'] += $ArrRemoveFileData
                }
            #endregion Build Return Array
        
            #region Output
                $EndTime = $(Get-Date -ErrorAction SilentlyContinue)
                $HashReturn['RemoveFile'].EndTime = $($EndTime).DateTime
                $HashReturn['RemoveFile'].ElapsedTime = $(New-TimeSpan -Start $StartTime -End $EndTime -ErrorAction SilentlyContinue)`
                ...
                #region Output Type
                    $ResultSet = $($HashReturn['RemoveFile']['Items'])
...
#endregion Remove-BluGenieFile (Function)